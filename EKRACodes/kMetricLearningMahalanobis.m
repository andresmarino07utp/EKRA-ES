...
elseif strcmp(options.init,'fda')
    options.lr = [1e-1,1e-2];
    Q = numel(unique(labels));
    [~,A_i] = FDA(X',labels,Q); %fda based
else
    A_i = options.init;
end
...
for ii = 1 : maxiter
    ...
    if fnew > fold %last A
        %fnew = fold;
        eta_start = eta_start-eta_start*.75;
        eta_end = eta_end-eta_end*.75;
    end
    if ii < maxiter/2
        eta = eta_start;
    else
